...
if CLIENT then
    ...
    function ENT:Think()
        ...
        if CurTime() > (self.LastSparkleTime + self.SparkleWait) then
            self.SparkleBuffer[self.SparkleIndex] = {
                self:GetPos()
                    + Vector(0, 0, self.SparkleHeightOffset)
                    + (VectorRand() * self.SparkleRadius),
                255,
            }
...
if SERVER then
    ...
    function ENT:Initialize()
        ...
        local mvObject = self:AddMarkerVision("greendemon_owner")
    ...
    function ENT:Think()
        ...
        for _, ply in ipairs(player.GetAll()) do
            if not ply:IsSpec() then
                ...
                if ply == self.activator then
                    dist = dist
                        * math.Clamp(
                            1 - GetConVar("sv_ttt2_greendemon_prefer_activator_bias"):GetFloat(),
                            0,
                            1
                        )
                end
        ...
        if IsValid(nearest) then
            if
                nearest_dist <= GetConVar("sv_ttt2_greendemon_kill_range"):GetInt()
                and nearest:Alive()
            then
                self:KillEm(nearest)
            else
                ...
                if phys then
                    phys:SetVelocityInstantaneous(
                        normVec * scale * GetConVar("sv_ttt2_greendemon_move_speed"):GetFloat()
                    )
                end
            end
        end
    ...
    function ENT:KillEm(v)
        ...
        self:RemoveMarkerVision("greendemon_owner")
        self:Remove()
    end
...
if CLIENT then
    ...
    hook.Add("TTT2RenderMarkerVisionInfo", "HUDDrawMarkerVisionGreenDemon", function(mvData)
        ...
        if not mvObject:IsObjectFor(ent, "greendemon_owner") then
            return
        end
        ...
        mvData:AddDescriptionLine(
            ParT("greendemon_marker_vision_target", { target = target }),
            color
        )
        ...
        mvData:SetCollapsedLine(
            ParT("greendemon_marker_collapsed", { distance = distance, target = target }),
            color
        )
    end)
end
