...
if TTT_TEAM_GLOW then is_reload = true end
TTT_TEAM_GLOW = TTT_TEAM_GLOW or {}
...
TTT_TEAM_GLOW.ShouldDrawOverheadIcon = function(ply)
...
TTT_TEAM_GLOW.ShouldDrawScoreboardTeam = function(ply)
	local tm = ply:GetTeam() or nil
...
TTT_TEAM_GLOW.ShouldDrawDisguised = function(ply)
...
TTT_TEAM_GLOW.ShouldDrawTargetIDPlayers = function(ply)
...
TTT_TEAM_GLOW.ShouldDrawPlayerNotSolo = function(ply)
...
TTT_TEAM_GLOW.last_peeped_entity = nil

TTT_TEAM_GLOW.PreDrawHalos = function()
	...
	for i = 1, #plys do
		...
		conditions.should_draw_overhead_icon = TTT_TEAM_GLOW.ShouldDrawOverheadIcon(ply)
		conditions.should_draw_scoreboard_team = TTT_TEAM_GLOW.ShouldDrawScoreboardTeam(ply)
		conditions.should_draw_disguised = TTT_TEAM_GLOW.ShouldDrawDisguised(ply)
		conditions.should_draw_target_id_players = TTT_TEAM_GLOW.ShouldDrawTargetIDPlayers(ply)
		conditions.should_draw_player_not_solo = TTT_TEAM_GLOW.ShouldDrawPlayerNotSolo(ply)
		...
		-- if peeped_entity ~= TTT_TEAM_GLOW.last_peeped_entity and IsValid(peeped_entity) and peeped_entity:IsPlayer() then
		-- 	print("glowdebug:", ply, "peeped", peeped_entity)
		-- 	PrintTable(conditions)
		-- end
		TTT_TEAM_GLOW.last_peeped_entity = peeped_entity
...
TTT_TEAM_GLOW.PostGamemodeLoaded = function()
	if GAMEMODE_NAME ~= "terrortown" then return end
	hook.Remove("PreDrawHalos", "TTT_TEAM_GLOW__PreDrawHalos")
	hook.Add("PreDrawHalos", "TTT_TEAM_GLOW__PreDrawHalos", TTT_TEAM_GLOW.PreDrawHalos)
end

if is_reload then
	print("reloaded: sh_role_glow_modified.lua")
	hook.Remove("PostGamemodeLoaded", "TTT_TEAM_GLOW__PostGamemodeLoaded")

	TTT_TEAM_GLOW.PostGamemodeLoaded()
end
hook.Add("PostGamemodeLoaded", "TTT_TEAM_GLOW__PostGamemodeLoaded", TTT_TEAM_GLOW.PostGamemodeLoaded)