...
local old_dlist = dlist
local curSearch = curSearch
local dcatlist = dcatlist
local dcats = dcats or {}
...
function PANEL:SelectPanel(pnl)
	...
	if pnl then pnl.PaintOver = nil end

	self.BaseClass.SelectPanel(self, pnl or {})
...
local categories = {
	...
	["Marker"] = {
		"weapon_ttt_paintgrenade",
		"weapon_ttt_paintawp",
		"weapon_ttt_paintknife",
		"weapon_ttt2_markerdefi",
	},
	["Ordinary Weapons"] = {
		"weapon_zm_revolver",
	...
	["Equipment"] = {
		...
		"weapon_ttt_defibrillator",
		"weapon_ttt_enderpearl",
	...
	["Special Grenades"] = {
		"weapon_ttt_ice",
		"weapon_ttt_liftgren",
		"weapon_ttt_bungernade",
		"weapon_ttt_confgrenade_s",
		"weapon_holyhand_grenade",
	...
	["Detective"] = {
		...
		"weapon_ttt_detective_toy_car",
		"weapon_ttt_homebat",
		"stungun",
		"weapon_ttt_ttt2_camera",
...
local function CreateEquipmentList(t)
	...
	local cats = {}
	local cat_panels = {}
	for cat_name, cat_value in pairs(categories) do
		cats[cat_name] = dcatlist:Add( cat_name )
		t.dcats[cat_name] = vgui.Create("EquipSelect", cats[cat_name])
		t.dcats[cat_name]:SetPos(0, 0)
		t.dcats[cat_name]:Dock( FILL )
		t.dcats[cat_name]:EnableVerticalScrollbar(true)
		t.dcats[cat_name]:EnableHorizontal(true)
		cats[cat_name]:SetContents(t.dcats[cat_name])
		cat_panels[cat_name] = {
			paneltablefav = {},
			paneltable = {},
		}
	end
	local item_distribution = {
	...
	for k = 1, #itms do
		...
		if t.search and string.find(string.lower(equipName), string.lower(t.search)) or not t.search then
			...
			local found_category = nil
			for cat_name, cat_value in pairs(categories) do
				if contains(cat_value, item.id) then
					-- item_distribution[item.id] = cat_name
					-- print("assigning", item.id, "to category", cat_name)
					found_category = cat_name
				end
			end
			if not categories[found_category] then
				print("uncategorized: ", item.id, item.name, item.PrintName)
				found_category = "Misc"
			end
			item_distribution[item.id].category = found_category
			parent = t.dcats[found_category]

			-- Create icon panel
			if item.ttt2_cached_material then
				ic = vgui.Create("LayeredIcon", parent)
			...
			elseif item.ttt2_cached_model then
				ic = vgui.Create("SpawnIcon", parent)
			...
			if ic.favorite then
				table.insert(cat_panels[item_distribution[item.id].category].paneltablefav, ic)
			else
				table.insert(cat_panels[item_distribution[item.id].category].paneltable, ic)
			end
			...
			item_distribution[item.id].ic = ic
		end
	end

	for cat_name, cat_value in pairs(categories) do
		-- add favorites first
		for _, panel in pairs(cat_panels[cat_name].paneltablefav) do
			t.dcats[cat_name]:AddPanel(panel)
		end

		-- non favorites second
		for _, panel in pairs(cat_panels[cat_name].paneltable) do
			t.dcats[cat_name]:AddPanel(panel)
		end
	end

	dcatlist:InvalidateLayout( true )
end
...
function TraitorMenuPopup()
	...
	-- icon size = 64 x 64
	old_dlist = vgui.Create("EquipSelect", dequip)
	-- local dlistw = 288
	old_dlist:SetPos(0, 0)
	old_dlist:SetSize(dlistw, dlisth)
	old_dlist:EnableVerticalScrollbar(true)
	old_dlist:EnableHorizontal(true)

	dcatlist = vgui.Create( "DCategoryList", old_dlist )
	dcatlist:Dock( FILL )

	coroutine.resume(currentEquipmentCoroutine, {notalive = notalive, dcats = dcats})
	...
	if notalive then
		...
		drolesel.OnSelect = function(panel, index, value)
			...
			coroutine.resume(currentEquipmentCoroutine, {role = RolenameToRole(value), search = dsearch:GetValue(), notalive = notalive, dcats = dcats})
		end
	...
	dsearch.OnValueChange = function(slf, text)
		...
		coroutine.resume(currentEquipmentCoroutine, {search = text, role = crole, notalive = notalive, dcats = dcats})
	end
	...
	old_dlist._selected = nil
	local WipeSelectionForAllBut = function(the_dcats, not_cat_panel)
		for cat_name, cat_panel in pairs(the_dcats) do
			if cat_panel ~= not_cat_panel then
				print("wiping out", cat_name)
				cat_panel:SelectPanel(nil)
			end
		end
	end

	-- couple panelselect with info
	for cat_name, cat_panel in pairs(dcats) do
		print(cat_name, cat_panel)
		if cat_panel and IsValid(cat_panel) then
			cat_panel.OnActivePanelChanged = function(self, _, new)
				...
				WipeSelectionForAllBut(dcats, self)
				old_dlist._selected = new
			end
		end

		-- select first
		-- cat_panel:SelectPanel(cat_panel:GetItems()[1])
	end

	-- prep confirm action
	dconfirm.DoClick = function()
		local pnl = old_dlist._selected
	...
	dsheet.OnTabChanged = function(_, _, new)
		if not IsValid(new) or not IsValid(old_dlist._selected) or new:GetPanel() ~= dequip then return end

		can_order = update_preqs(old_dlist._selected.item)

		old_dlist._selected.item.disabledBuy = not can_order
	...
	dfav.DoClick = function()
		...
		-- Reload item list
		coroutine.resume(currentEquipmentCoroutine, {role = role, search = curSearch, notalive = notalive, dcats = dcats})
	end
