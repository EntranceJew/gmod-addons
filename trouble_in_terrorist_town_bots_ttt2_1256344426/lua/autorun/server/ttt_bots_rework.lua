...
hook.Add("TTTBeginRound", "TTTBotBeginRound", function()
	...
	if GetConVar("ttt_bot_rdm"):GetInt() == 1 then
		local bots = GetInnocentBots()
		-- print("fuckinsuck")
		-- PrintTable(bots)
		table.Shuffle(bots)
		rdmer = table.remove(bots)
	...
	for i,bot in pairs(player.GetAll()) do
		if !bot:IsBot() then continue end
		bot.massinit = false
		bot.c4 = false
		bot.target = nil
		bot.willroam = true
		...
		--bot.target = table.Random(player.GetAll())
	end
...
function Meta:AttackFunction(cmd)
	local bot = self
	if (rdmer == bot) and bot.target == nil then
...
--if gmod.GetGamemode().Name != "Trouble in Terrorist Town" then return end
	for b, bot in pairs(player.GetAll()) do
		if !bot:IsBot() then continue end
...
local function addbot(prof, c, p)
	local profile = nil
	local _x = false
	if (prof != nil) and IsValid(prof) and prof:IsPlayer() then
		prof = nil
	end
	local n = 1
	...
	for i = 1, n, 1 do
		if prof == nil then
			-- we have no desired profile
			-- the only profile we want, is of a profile not in use
			local shuffled_profiles = table.Copy(profiles)
			table.Shuffle(shuffled_profiles)

			-- build an index of exiting player names
			local existing_names = {}
			for _, ply in ipairs(player.GetAll()) do
				existing_names[ ply:Nick() ] = true
			end

			-- now just find one not in use
			for _, profile_item in pairs(shuffled_profiles) do
				if !existing_names[ profile_item.name ] then
					profile = profile_item
					break
				end
			end

			-- if all else fails, just use the first one again
			if profile == nil then
				profile = shuffled_profiles[1]
			end
		else
			profile = prof
		end
...
--[[local function addbot(prof, c, p)
	...
	-- if we already have a profile that matches a player find a new one that doesn't?
	if (IsValid(Prof) and prof:IsPlayer()) then
		prof = nil
	end

	-- check that p is valid?
	local nBotsToCreate = 0
	if (p != nil and p[1] != nil) then
		nBotsToCreate = tonumber(p[1])
	end

	for i = 0, nBotsToCreate, 1 do
		if prof == nil then
			-- if we find a player with the same nick as the profile.name repeat
			-- this until we get a profile that doesn't match any of the player nicks?
			local bBreakLoop = false
			while (not bBreakLoop) do
				...
				-- iterated over all the players and couldn't find a Nick matching the profile.name success
				bBreakLoop = true
			end
		else
			profile = prof -- already have a profile, we're good to create a bot.
		end
...
local function getBotsAndPlayers()
	-- local num_bots = 0
	-- local num_plys = 0
	...
	for _, ply in ipairs(plys) do
		if ply:IsBot() then
			...
			-- num_plys = num_plys + 1
			table.insert(not_bots, ply)
		end
	end

	return bots, not_bots
end
...
local function updateQuota()
	...
	if quota > 0 then
		...
		if quota < num_plys then -- if there are more players than the quota
			-- if there are any bots, remove 1 of them
			if num_bots > 0 then
				bots[1]:Kick("Quota reached; kicking bot")
			end
		elseif quota > num_plys then -- if there are less players than the quota
			-- add a bot!
			addbot()
		end
	end
...
net.Receive( "RequestKickBots", function( len, ply )
	local n = net.ReadInt(32)
	local bots = getBotsAndPlayers()
	if #bots == 0 then return end
	if ply:IsSuperAdmin() then
		for i = 1, n do
			local bot = table.Random(bots)
			table.RemoveByValue(bots, bot)
			if bot != nil then
...
concommand.Add("ttt_bot_add", function() addbot() end) -- Broken Console Command, don't uncomment
...
function Meta:generateWanderPos(maxVisPlayers)
	...
	for try = 1, tries, 1 do
		...
		-- search for less ambitious places to be
		pos = pos * ((tries - try) / tries)
		...
		for _,v in pairs(bombs) do -- 750
			if IsValid(v) and v:GetPos():Distance(pos) < 750 then
				dangerous = true
			end
		end
		if (ply:GetRoleString() == "traitor") then
			for _,v in pairs(tbombs) do -- 750
				if IsValid(v) and v:GetPos():Distance(pos) < 750 then
					dangerous = true
				end
			end
		...
		-- todo: maxvisplayers
		if util.IsInWorld(pos) and (!dangerous) then
			found_pos = true
			break
		end
	end
...
function GetInnocentBots()
	local bots = {}
	for i,v in pairs(player.GetAll()) do
		-- print("are we innocent?", v)
		if v:IsBot() and v:GetSubRole() == ROLE_INNOCENT and v:Health() > 0 and !v:IsSpec() then
			table.insert(bots, v)
		end
	end
	return bots
...
--if gmod.GetGamemode().Name != "Trouble in Terrorist Town" then return end
	local bots = getBotsAndPlayers()
	local v = table.Random(bots)
	if v != nil then
		if v:Health() > 0 then
			v:BotSay(ttt_bot_gchat(v.personality, "silly", table.Random(bots):Nick()))
		end
	end
end)
...
timer.Create("TTTDetectTraitorWeapons", 1, 0, function()
	...
	if roundIsActive then
		for i,bot in pairs(player.GetAll()) do
			...
			--------------------
			--------------------
			for m,view in pairs(player.GetAll()) do
				if not view:IsBot() then continue end
